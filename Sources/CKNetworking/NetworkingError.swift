...
public struct NetworkingError: Error, LocalizedError {
    ...
    public init(code: Int, message: String) {
        self.status = Status(rawValue: code) ?? .unknown
        self.localizedDescription = message
    }
    
    public init(status: Status) {
        self.status = status
        self.localizedDescription = "\(status)"
    }
    
    public init(urlError: URLError) {
        self.status = Status(rawValue: urlError.errorCode) ?? .unknown
        self.localizedDescription = urlError.localizedDescription
    }
